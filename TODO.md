...
- [x] ~~Nutzer sollen mit einer (eine, nicht mehrere) eigenen CSS-Datei in ~/.config/wsr/style.css die Default css überschreiben. Ein parameter --style|-s erlaubt das explizite anfügen einer css-Datei.~~ (Implementiert: `-s/--style` in `main.py`, `resolve_style_path()` in `config.py`, ReportGenerator erweitert)
- [x] ~~das html lang-Attribut in report_generator soll die systemeinstellungen oder die einstellungen aus dem parameter --lang übernehmen.~~ (Implementiert: ReportGenerator nutzt jetzt `lang` Parameter aus i18n)
- [x] ~~Nutzer sollen folgendes für die Speicherung von Bildern entscheiden: a) Ausgabe-Format (png,jpg,webp) --image-format {png|jpg|webp} b) Qualität --image-quality 0.1-1.0 (jpg und webp)~~ (Implementiert: `--image-format` und `--image-quality` in `main.py` und `report_generator.py`)
...
| --style,-s             | String (Pfad)                | `~/.config/wsr/style.css` (falls vorhanden)    | Pfad zu einer zusätzlichen/eigenen CSS-Datei; überschreibt die eingebauten Standard-Styles  | Done |
| --image-format         | String (enum: png\|jpg\|webp) | `png`                                           | Bildformat der Screenshots                                                                   | Done |
| --image-quality        | Float (0.1-1.0)             | `0.9`                                           | Qualitätsfaktor für `jpg`/`webp`                                                             | Done |
